...
test('Chaque salle a un gamemaster formé', () => {
    const sessions = assignGamemasters(ROOMS, GAMEMASTERS);
    expect(sessions.length).toBe(ROOMS.length);
    sessions.forEach(session => {
        expect(session.gamemaster).toBeDefined();
        expect(GAMEMASTERS.find(gm => gm.id === session.gamemaster.id).trained_rooms)
            .toContain(session.room.id);
    });
});

test('Aucune solution possible', () => {
    const impossibleRooms = [
        { id: 13, name: "Nouvelle salle 1" },
        { id: 14, name: "Nouvelle salle 2" },
    ];
    expect(() => assignGamemasters(impossibleRooms, GAMEMASTERS)).toThrow('Aucune solution possible');
});
